...
SELECT d.*, a.acl
FROM djet.data_node d
LEFT JOIN (
	SELECT d.node_id, array_agg(array_to_string(p.datacolumns, ',')) acl
	FROM djet.data_node d
	JOIN djet.data_node p ON n.parent=p.id
	JOIN djet.basetype b ON p.basetype_id=b.id
	WHERE d.node_path=''
	AND b.name='acl'
	GROUP BY d.node_id
) a USING (node_id)
...
CREATE OR REPLACE VIEW data_node_acl AS (
	...
	LEFT JOIN (
		SELECT d.node_id, array_agg(array_to_string(p.datacolumns, ',')) acl
		FROM data_node d
		JOIN djet.node_tree n ON d.node_id=n.child
		JOIN djet.data_node p ON n.parent=p.node_id
		JOIN djet.basetype b ON p.basetype_id=b.id
		AND b.name='acl'
		GROUP BY d.node_id
	) a USING (node_id)
